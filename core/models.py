...
class FourierEmbedding(torch.nn.Module):
    ...
    def forward(self, x):
        x = x.view(-1, 1)
        freqs = (2 * np.pi * self.freqs).to(dtype=x.dtype, device=x.device)
        x_proj = x * freqs.view(1, -1)
...
class SongUNet(torch.nn.Module):
    ...
    def forward(
        self, x, noise_labels, class_labels, time_labels=None, augment_labels=None
    ):
        # Mapping
        ...
        if self.map_time is not None and time_labels is not None:
            ...
            tmp = tmp.reshape(tmp.shape[0], 2, -1).flip(1).reshape(*tmp.shape)
            if self.training and self.label_dropout:
                tmp = tmp * (
                    torch.rand([x.shape[0], 1], device=x.device) >= self.label_dropout
                ).to(tmp.dtype)
...
class DetPrecond(torch.nn.Module):
    def __init__(
        self, img_resolution, img_channels, out_channels, filters=128, label_dropout=0
    ):
        super().__init__()
