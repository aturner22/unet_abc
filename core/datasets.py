...
class ERA5Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.static_fields is not None and input_tensor.shape[0] > n_met_channels:
            ...
            expanded_mean = self.mean.repeat(n_actual_fields).reshape(-1, 1, 1)
            expanded_std = self.std_dev.repeat(n_actual_fields).reshape(-1, 1, 1)
        ...
        target_tensor = (
            target_tensor - self.mean.reshape(-1, 1, 1)
        ) / self.std_dev.reshape(-1, 1, 1)
